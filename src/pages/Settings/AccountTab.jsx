...
import { FIELD_REQUIRED_MESSAGE, singleFileValidator } from '~/utitls/validators'
...
function AccountTab() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    defaultValues: initialGeneralForm // Khởi tạo giá trị mặc định cho form
  }) 
  ...
  const submitChangeGeneralInformation = (data) => {
    ...
    toast.promise( dispatch(updateUserAPI({displayName}) ), { pending :'Updating...'})
            .then(res => {
               //doan nay pk kiem tra neu khong co loi(update thanh cong )  thi moi thuc hien hanh dong can thiet
              if(!res.error) {
  ...
  const uploadAvatar = (e) => {
    ...
    toast.promise( dispatch(updateUserAPI(reqData) ), { pending :'Updating...'})
            .then(res => {
              ...
              //lưu ý , dù có lỗi hay không thì cũng phải reset lại input file để tránh trường hợp người dùng upload lại cùng 1 file
